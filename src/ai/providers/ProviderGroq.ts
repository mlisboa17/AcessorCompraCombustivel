...
export class ProviderGroq implements AIProvider {
  public readonly name = "Groq" as const;

  async generateResponse(input: string, context?: AIContext): Promise<string> {
    const apiUrl = readRequiredEnv("GROQ_API_URL", this.name);
    const apiKey = readRequiredEnv("GROQ_API_KEY", this.name);
    ...
    if (!content || typeof content !== "string") {
      throw new AIProviderError("Resposta invalida recebida do Groq.", this.name, false);
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new AIProviderError(
          `Groq falhou com HTTP ${response.status}.`,
          this.name,
      ...
      throw new AIProviderError(
        isTimeout ? "Timeout ao chamar Groq." : "Falha de rede ao chamar Groq.",
        this.name,
        true,
      );
    } finally {
